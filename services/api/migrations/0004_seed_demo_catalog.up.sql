-- Seeds a small real catalog (stores/categories/products/inventory) so a
-- fresh database -- including this project's live Supabase instance, which
-- had zero products until now -- shows a working catalog instead of empty
-- states everywhere. Copied from infrastructure/database/seed/0001_seed.sql
-- (the canonical source, also usable standalone via psql for local dev),
-- minus its own BEGIN/COMMIT -- golang-migrate already wraps every
-- migration file in one transaction, so a nested BEGIN/COMMIT here would
-- commit early and break that. Every INSERT below is idempotent
-- (ON CONFLICT DO NOTHING / WHERE EXISTS guards), so this is safe to run
-- on every deploy, including ones that already ran it, and safe to run
-- against a database that already has real seller-submitted products --
-- it will never overwrite or duplicate anything.
